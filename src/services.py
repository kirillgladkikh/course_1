...
def investment_bank(
    ...
    def calculate_difference(transaction: Dict, limit: Decimal) -> Decimal:
        """
        Вычисляет разницу между округлённой и исходной суммой платежа.

        Функция берёт сумму платежа из транзакции, округляет её вверх до ближайшего значения,
        кратного заданному лимиту (с помощью функции `round_to_limit`), а затем вычисляет
        разницу между округлённой суммой и исходной.

        Параметры:
        ----------
        transaction : Dict
            Словарь, представляющий транзакцию. Должен содержать ключ 'payment_amount',
            значение которого имеет тип `Decimal` и обозначает исходную сумму платежа.
        limit : Decimal
            Лимит (шаг округления) — значение, к которому должна быть приведена сумма платежа
            при округлении. Определяет кратность, до которой будет округляться `payment_amount`.

        Возвращает:
        -----------
        Decimal
            Разница между округлённой суммой платежа (вверх до кратного `limit`) и исходной
            суммой платежа. Значение всегда неотрицательное (так как округление выполняется вверх).

        Примечания:
        ---------
        - Для округления используется функция `round_to_limit`, которая всегда округляет вверх
          до ближайшего кратного значения.
        - Если исходная сумма уже кратна `limit`, разница будет равна нулю.
        """
...
def services_page(input_data: Union[Timestamp, str] = Timestamp('2021-12-31 16:44:00')) -> str:
    ...
    2. Определяет целевой месяц для расчёта в формате 'ГГГГ‑ММ' на основе параметра `input_data`.
    3. Задаёт параметры расчёта:
       - `coin_limit` — максимальный размер отчисления за одну транзакцию (по умолчанию 50 руб.);
       - `coin_month_str` — целевой месяц в формате 'ГГГГ‑ММ' (на основе `input_data`).
    4. Преобразует лимит отчислений в тип `Decimal` для обеспечения точности вычислений.
    5. Вызывает функцию `investment_bank()` для расчёта суммы в «Инвесткопилке»,
       передавая ей:
       - список транзакций;
       - целевой месяц;
       - лимит отчисления в типе `Decimal`.
    6. Логирует результат расчёта, включая:
       - итоговую сумму в «Инвесткопилке»;
       - установленный лимит отчисления;
       - период расчёта (месяц и год).
    7. Возвращает строку с рассчитанной суммой.

    Параметры расчёта:
    - coin_limit (int): максимальный размер отчисления за одну транзакцию (по умолчанию 50 руб.).
    - coin_month_str (str): месяц и год для расчёта в формате 'ГГГГ‑ММ' (по умолчанию '2021‑12').

    Параметры:
        input_data (Union[Timestamp, str]):
            Дата, на основе которой определяется целевой месяц для расчёта.
            Может быть строкой в формате даты или объектом `Timestamp`.
    ...
    Возвращаемое значение:
        str: строка, содержащая рассчитанную сумму в «Инвесткопилке».
