...
def reports_page(input_data: Union[Timestamp, str] = Timestamp('2021-12-31 16:44:00')) -> pd.DataFrame:
    ...
    Параметры:
        input_data (Union[Timestamp, str]):
            Дата, относительно которой определяется отчётный период (последние три месяца).
    ...
    Возвращаемое значение:
        pd.DataFrame:
            ...
            - `day_type` — тип дня (`workday` или `weekend`);
            - `average_spending` — средняя сумма трат за день указанного типа.
